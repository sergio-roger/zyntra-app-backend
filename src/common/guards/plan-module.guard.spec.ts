...
import { PlanModule, ModuleAccessLevel } from '../../modules/auth/entities/plan-module.entity';
...
const adminSession = {
  id: 'biz-1', name: 'Acme', email: 'acme@test.com',
  plan_id: PLAN_ID, plan_status: 'active',
  crm_user_id: null, role: 'admin',
};
// CRM manager: belongs to the same business, plan_id is inherited from it
const managerSession = { ...adminSession, crm_user_id: 'usr-mgr-1', role: 'manager' };
// CRM agent: same business → same plan_id
const agentSession   = { ...adminSession, crm_user_id: 'usr-agt-1', role: 'agent' };
// Edge-case: business created before plan was seeded
const noPlanSession  = { ...adminSession, plan_id: null };

// ── Test infrastructure ───────────────────────────────────────────────────────

function makeContext(user: any, method = 'GET'): ExecutionContext {
  return {
...
function makeDataSource(
  ...
  const pmFindOne = jest.fn(({ where }: any) => {
  ...
  const menuFindOne = jest.fn(({ where }: any) => {
    if (where.key in menuParents) {
      return Promise.resolve({ key: where.key, parent_key: menuParents[where.key] });
    }
...
function makeGuard(
  ...
  return new PlanModuleGuard(reflector, makeDataSource(modules, menuParents) as any);
}
...
describe('PlanModuleGuard', () => {
  ...
  describe('No @RequiresModule decorator', () => {
    it('returns true — endpoint is freely accessible', async () => {
      ...
      await expect(guard.canActivate(makeContext(adminSession))).resolves.toBe(true);
    });
  });
  ...
  describe('Missing / invalid session', () => {
    ...
    it('returns false when user has no plan_id (business created before plan seed)', async () => {
      const guard = makeGuard('crm', { crm: FULL });
      await expect(guard.canActivate(makeContext(noPlanSession))).resolves.toBe(false);
    });
  });
  ...
  describe('Business-owner session — crm_user_id: null, role: admin', () => {
    ...
    describe('Module access → FULL', () => {
      it('GET allowed', async () => {
        const guard = makeGuard('crm', { crm: FULL });
        await expect(guard.canActivate(makeContext(adminSession, 'GET'))).resolves.toBe(true);
      });

      it('POST allowed', async () => {
        const guard = makeGuard('crm', { crm: FULL });
        await expect(guard.canActivate(makeContext(adminSession, 'POST'))).resolves.toBe(true);
      });

      it('PATCH allowed', async () => {
        const guard = makeGuard('crm', { crm: FULL });
        await expect(guard.canActivate(makeContext(adminSession, 'PATCH'))).resolves.toBe(true);
      });

      it('DELETE allowed', async () => {
        const guard = makeGuard('crm', { crm: FULL });
        await expect(guard.canActivate(makeContext(adminSession, 'DELETE'))).resolves.toBe(true);
      });
    });
    ...
    describe('Module access → LOCKED', () => {
      it('throws ForbiddenException', async () => {
        ...
        await expect(guard.canActivate(makeContext(adminSession))).rejects.toThrow(ForbiddenException);
      });

      it('response has code MODULE_LOCKED and the module key', async () => {
        const guard = makeGuard('crm', { crm: LOCKED });
        const err = await catchForbidden(guard, makeContext(adminSession));
        expect(err.getResponse()).toMatchObject({ code: 'MODULE_LOCKED', module: 'crm' });
      });
    });
    ...
    describe('Module access → READ_ONLY', () => {
      ...
      it.each(['POST', 'PATCH', 'PUT', 'DELETE'])(
        ...
        async (method) => {
          const guard = makeGuard('settings', { settings: READ_ONLY });
          const err = await catchForbidden(guard, makeContext(adminSession, method));
          expect(err.getResponse()).toMatchObject({ code: 'MODULE_READ_ONLY', module: 'settings' });
        },
      );
    });
  });
  ...
  describe('CRM user session — crm_user_id set, role: manager', () => {
    it('FULL → GET allowed', async () => {
      const guard = makeGuard('crm', { crm: FULL });
      await expect(guard.canActivate(makeContext(managerSession, 'GET'))).resolves.toBe(true);
    });

    it('FULL → POST allowed', async () => {
      const guard = makeGuard('crm', { crm: FULL });
      await expect(guard.canActivate(makeContext(managerSession, 'POST'))).resolves.toBe(true);
    });
    ...
    it('READ_ONLY → PATCH throws MODULE_READ_ONLY', async () => {
      const guard = makeGuard('inbox_channels', { inbox_channels: READ_ONLY });
      const err = await catchForbidden(guard, makeContext(managerSession, 'PATCH'));
  ...
  describe('CRM user session — crm_user_id set, role: agent', () => {
    it('FULL → allowed', async () => {
      const guard = makeGuard('crm', { crm: FULL });
      await expect(guard.canActivate(makeContext(agentSession))).resolves.toBe(true);
    });

    it('LOCKED → throws MODULE_LOCKED', async () => {
      const guard = makeGuard('funnels', { funnels: LOCKED });
      await expect(guard.canActivate(makeContext(agentSession))).rejects.toThrow(ForbiddenException);
    });
  ...
  describe('Parent-key inheritance (sub-key not explicit in plan_modules)', () => {
    ...
    it('crm_contacts inherits LOCKED from crm → throws MODULE_LOCKED', async () => {
      ...
      const err = await catchForbidden(guard, makeContext(adminSession));
      expect(err.getResponse()).toMatchObject({ code: 'MODULE_LOCKED', module: 'crm_contacts' });
    });
    ...
    it('crm_tags inherits READ_ONLY → PATCH blocked', async () => {
      const guard = makeGuard('crm_tags', { crm: READ_ONLY }, crmMenuParents);
      const err = await catchForbidden(guard, makeContext(adminSession, 'PATCH'));
      expect(err.getResponse()).toMatchObject({ code: 'MODULE_READ_ONLY', module: 'crm_tags' });
    });

    it('explicit sub-key takes precedence over parent (crm_tags: FULL overrides crm: LOCKED)', async () => {
      const guard = makeGuard('crm_tags', { crm: LOCKED, crm_tags: FULL }, crmMenuParents);
      await expect(guard.canActivate(makeContext(adminSession))).resolves.toBe(true);
    });
    ...
    it('parent inheritance works for manager session', async () => {
      ...
      await expect(guard.canActivate(makeContext(managerSession))).resolves.toBe(true);
    });
  ...
  describe('BrandStart plan — CRM locked, settings read-only', () => {
    ...
    it('settings → READ_ONLY, DELETE blocked', async () => {
      const guard = makeGuard('settings', modules, menuParents);
      const err = await catchForbidden(guard, makeContext(adminSession, 'DELETE'));
      expect(err.getResponse()).toMatchObject({ code: 'MODULE_READ_ONLY', module: 'settings' });
    });
    ...
    it('settings_users → LOCKED (explicit override, not inheriting READ_ONLY from parent)', async () => {
      ...
      const err = await catchForbidden(guard, makeContext(adminSession));
      expect(err.getResponse()).toMatchObject({ code: 'MODULE_LOCKED', module: 'settings_users' });
    });

    it('crm locked for manager session too', async () => {
      const guard = makeGuard('crm', modules, menuParents);
      await expect(guard.canActivate(makeContext(managerSession))).rejects.toThrow(ForbiddenException);
    });
  });
  ...
  describe('Impulse Pro plan — funnels and avatar locked', () => {
    ...
    it('funnels_dashboard inherits LOCKED from funnels (admin)', async () => {
      ...
      const err = await catchForbidden(guard, makeContext(adminSession));
      expect(err.getResponse()).toMatchObject({ code: 'MODULE_LOCKED', module: 'funnels_dashboard' });
    });
    ...
    it('inbox_channels → READ_ONLY, POST blocked', async () => {
      const guard = makeGuard('inbox_channels', modules, menuParents);
      await expect(guard.canActivate(makeContext(adminSession, 'POST'))).rejects.toThrow(ForbiddenException);
    });
  ...
  describe('Core Digital plan — all root modules FULL', () => {
    const modules: Record<string, ModuleAccessLevel> = {
      dashboard: FULL, crm: FULL, agents_ia: FULL,
      inbox: FULL, funnels: FULL, avatar: FULL,
      analytics: FULL, billing: FULL, settings: FULL,
    };
    const menuParents: Record<string, string> = {
      funnels_dashboard: 'funnels',
      funnels_builder:   'funnels',
    ...
    it('funnels_builder inherits FULL → DELETE allowed (agent)', async () => {
      const guard = makeGuard('funnels_builder', modules, menuParents);
      await expect(guard.canActivate(makeContext(agentSession, 'DELETE'))).resolves.toBe(true);
    });
