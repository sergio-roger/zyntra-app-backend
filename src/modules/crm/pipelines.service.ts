...
export class PipelinesService {
  ...
  async list(business: Business, user: UserContext): Promise<Pipeline[]> {
    ...
    // Manager/Agent: solo pipelines con equipo asignado donde el usuario es miembro
      ...
      if (plan && plan.pipelineLimit > 0) {
        ...
        if (current >= plan.pipelineLimit) {
          throw new ForbiddenException(
            `Tu plan "${plan.name}" permite hasta ${plan.pipelineLimit} pipeline(s). Actualiza tu plan para crear más.`,
          );
        }
      }
    }
